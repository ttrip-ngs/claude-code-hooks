...
✅ **作業完了通知**
$env_info
作業時間：${working_time:-"不明"}
指示：${instructions:-"指示未設定"}
作業内容：${result_summary:-"詳細記録なし"}$(if [[ -n "$last_commit" ]]; then echo "
最新コミット：$last_commit"; fi)
$session_info
EOF
}
...
generate_result_summary() {
    ...
    if git rev-parse --is-inside-work-tree &> /dev/null 2>&1; then
        ...
        if [[ -n "$recent_commits" ]]; then
            summary="最近のコミット（1時間以内）:$recent_commits"
        fi

        # 変更されたファイルの一覧
        local changed_files=$(git diff --name-only HEAD~1 2>/dev/null | head -5 | tr '\n' ', ' | sed 's/,$//')
        if [[ -n "$changed_files" ]]; then
            summary="$summary | 変更ファイル:$changed_files"
        fi
    fi

    # 作業ディレクトリ内の最近作成されたファイル
    local recent_files=$(find . -type f -mmin -60 -not -path './.git/*' 2>/dev/null | head -3 | tr '\n' ', ' | sed 's/,$//')
    if [[ -n "$recent_files" ]]; then
        summary="$summary | 最新ファイル:$recent_files"
    fi
